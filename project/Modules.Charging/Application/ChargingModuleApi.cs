...
internal sealed class ChargingModuleApi : IChargingModuleApi
{
    ...
    public Task<MaintenanceContract> CreateMaintenanceAsync(MaintenanceContract maintenance, CancellationToken ct = default, string? actorUserName = null) => _service.CreateMaintenanceAsync(maintenance, ct, actorUserName);
    public Task<bool> UpdateMaintenanceStatusAsync(Guid maintenanceId, EMaintenanceStatus status, string? notes, DateTime? resolvedAtUtc, CancellationToken ct = default, string? actorUserName = null) => _service.UpdateMaintenanceStatusAsync(maintenanceId, status, notes, resolvedAtUtc, ct, actorUserName);
    public Task<bool> AssignMaintenanceAsync(Guid maintenanceId, Guid? assignedToUserId, CancellationToken ct = default, string? actorUserName = null) => _service.AssignMaintenanceAsync(maintenanceId, assignedToUserId, ct, actorUserName);
