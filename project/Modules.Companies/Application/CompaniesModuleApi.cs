...
internal sealed class CompaniesModuleApi : ICompaniesModuleApi
{
    ...
    public Task<UserCompanyMembershipContract?> SelectActiveCompanyAsync(Guid userId, Guid companyId, string actorUserName, CancellationToken ct = default) => _service.SelectActiveCompanyAsync(userId, companyId, actorUserName, ct);
    public Task<bool> HasCompanyRoleAsync(Guid companyId, Guid userId, string minimumRole, CancellationToken ct = default) => _service.HasCompanyRoleAsync(companyId, userId, minimumRole, ct);
    ...
    public Task<CompanyUserMutationResultContract> UpdateCompanyMembershipRoleWithGuardsAsync(Guid companyId, Guid membershipId, string role, CancellationToken ct = default) => _service.UpdateCompanyMembershipRoleWithGuardsAsync(companyId, membershipId, role, ct);
    public Task<CompanyUserMutationResultContract> DeactivateCompanyMembershipWithGuardsAsync(Guid companyId, Guid membershipId, CancellationToken ct = default) => _service.DeactivateCompanyMembershipWithGuardsAsync(companyId, membershipId, ct);
    public Task<AddCompanyUserResultContract> AddCompanyUserAsync(AddCompanyUserContract request, CancellationToken ct = default) => _service.AddCompanyUserAsync(request, ct);
    ...
    public Task<IReadOnlyCollection<CompanyPromotionContract>> GetSystemPromotionsAsync(CancellationToken ct = default) => _service.GetSystemPromotionsAsync(ct);
    public Task<CompanyPromotionContract?> GetSystemPromotionAsync(Guid promotionId, CancellationToken ct = default) => _service.GetSystemPromotionAsync(promotionId, ct);
    public Task<PromotionOperationResultContract> CreateSystemPromotionAsync(UpsertCompanyPromotionContract request, CancellationToken ct = default) => _service.CreateSystemPromotionAsync(request, ct);
    public Task<PromotionOperationResultContract> UpdateSystemPromotionAsync(Guid promotionId, UpsertCompanyPromotionContract request, CancellationToken ct = default) => _service.UpdateSystemPromotionAsync(promotionId, request, ct);
    public Task<bool> DeleteSystemPromotionAsync(Guid promotionId, CancellationToken ct = default) => _service.DeleteSystemPromotionAsync(promotionId, ct);
    ...
    public Task LogAuditMutationAsync(Guid companyId, string userName, string entityName, Guid entityId, string action, string? changesJson = null, CancellationToken ct = default)
        => _service.LogAuditMutationAsync(companyId, userName, entityName, entityId, action, changesJson, ct);
