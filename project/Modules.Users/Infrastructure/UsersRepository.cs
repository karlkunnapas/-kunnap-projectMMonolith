...
internal sealed class UsersRepository : IUsersRepository
{
    ...
    public async Task<IReadOnlyCollection<UserVehicleDto>> GetUserVehiclesAsync(Guid userId, CancellationToken ct = default)
    {
    ...
    public async Task<UserVehicleDto?> GetVehicleForUserAsync(Guid vehicleId, Guid userId, CancellationToken ct = default)
    {
    ...
    public async Task<UserVehicleDto> CreateVehicleAsync(Guid userId, CreateUserVehicleDto request, CancellationToken ct = default)
    {
    ...
    public async Task<UserVehicleDto?> UpdateVehicleAsync(Guid vehicleId, Guid userId, UpdateUserVehicleDto request, CancellationToken ct = default)
    {
        var vehicle = await _dbContext.Vehicles
    ...
    public async Task<bool> UpdateUserProfileAsync(Guid userId, UpdateUserProfileDto request, CancellationToken ct = default)
    {
    ...
    public async Task<ChangePasswordResultDto> ChangePasswordAsync(Guid userId, string currentPassword, string newPassword, CancellationToken ct = default)
    {
        var user = await _dbContext.Users.FirstOrDefaultAsync(u => u.Id == userId, ct);
        if (user == null)
        {
            return ChangePasswordResultDto.Fail("NOT_FOUND", "User not found.");
        }

        if (string.IsNullOrWhiteSpace(user.PasswordHash))
        {
            return ChangePasswordResultDto.Fail("INVALID_PASSWORD", "Current password is invalid.");
        }
    ...
    public async Task<string?> IssueRefreshTokenAsync(IssueRefreshTokenDto request, CancellationToken ct = default)
    {
    ...
    public async Task<RenewRefreshTokenResultDto> RenewRefreshTokenAsync(Guid userId, string refreshToken, DateTime newExpirationUtc, CancellationToken ct = default)
    {
        var normalizedRefreshToken = refreshToken?.Trim() ?? string.Empty;
        if (string.IsNullOrWhiteSpace(normalizedRefreshToken))
        {
            return RenewRefreshTokenResultDto.Fail("INVALID", "Refresh token is invalid.");
        }
        ...
        if (matches.Count == 0)
        {
            return RenewRefreshTokenResultDto.Fail("INVALID_OR_EXPIRED", "Refresh token is invalid or expired.");
        }

        if (matches.Count != 1)
        {
            return RenewRefreshTokenResultDto.Fail("MULTIPLE_VALID", "More than one valid refresh token found.");
        }
    ...
    public async Task<RegisterCustomerResultDto> RegisterCustomerAsync(RegisterCustomerDto request, CancellationToken ct = default)
    {
        ...
        if (existing)
        {
            return RegisterCustomerResultDto.Fail("DUPLICATE_EMAIL", "A user with this email already exists.");
        }
        ...
        if (!createResult.Succeeded)
        {
            ...
            return RegisterCustomerResultDto.Fail("USER_CREATION_FAILED", message);
        }
    ...
    public async Task<TwoFactorStatusDto> GetTwoFactorStatusAsync(Guid userId, CancellationToken ct = default)
    {
        ...
        if (user == null)
        {
            return new TwoFactorStatusDto { UserExists = false };
        }
    ...
    public async Task<TwoFactorSetupDto> StartTwoFactorSetupAsync(Guid userId, string appName, CancellationToken ct = default)
    {
        var user = await _userManager.FindByIdAsync(userId.ToString());
        if (user == null)
        {
            return new TwoFactorSetupDto { UserExists = false };
        }
    ...
    public async Task<TwoFactorRecoveryCodesDto> EnableTwoFactorAsync(Guid userId, string verificationCode, CancellationToken ct = default)
    {
        var user = await _userManager.FindByIdAsync(userId.ToString());
        if (user == null)
        {
            return new TwoFactorRecoveryCodesDto { UserExists = false };
        }
    ...
    public async Task<TwoFactorRecoveryCodesDto> RegenerateRecoveryCodesAsync(Guid userId, CancellationToken ct = default)
    {
        var user = await _userManager.FindByIdAsync(userId.ToString());
        if (user == null)
        {
    ...
    public async Task<DeleteAccountResultDto> DeleteAccountAsync(Guid userId, string? password, CancellationToken ct = default)
    {
        var user = await _userManager.FindByIdAsync(userId.ToString());
        if (user == null)
        {
            return DeleteAccountResultDto.Fail("NOT_FOUND", "User not found.");
        }
        ...
        if (hasPassword)
        {
            ...
            {
                return DeleteAccountResultDto.Fail("PASSWORD_REQUIRED", "Password is required.");
            }

            var validPassword = await _userManager.CheckPasswordAsync(user, password);
            if (!validPassword)
            {
                return DeleteAccountResultDto.Fail("INVALID_PASSWORD", "Incorrect password.");
            }
        }
        ...
        if (!result.Succeeded)
        {
            return DeleteAccountResultDto.Fail(
                "DELETE_FAILED",
                string.Join(", ", result.Errors.Select(e => e.Description)));
        }

        return DeleteAccountResultDto.Ok();
    }

    public async Task<AuthenticateUserResultDto> AuthenticateByEmailAsync(string email, string password, CancellationToken ct = default)
    {
        var normalizedEmail = email.Trim();
        var user = await _userManager.FindByEmailAsync(normalizedEmail);
        if (user == null)
        {
            return AuthenticateUserResultDto.Fail("USER_NOT_FOUND", "User not found.");
        }

        var validPassword = await _userManager.CheckPasswordAsync(user, password);
        if (!validPassword)
        {
            return AuthenticateUserResultDto.Fail("INVALID_PASSWORD", "Invalid password.");
        }

        return AuthenticateUserResultDto.Ok(user.Id);
    }

    public async Task<RegisterBasicUserResultDto> RegisterBasicUserAsync(RegisterBasicUserDto request, CancellationToken ct = default)
    {
        var existing = await _userManager.FindByEmailAsync(request.Email.Trim());
        if (existing != null)
        {
            return new RegisterBasicUserResultDto
            {
                Success = false,
    ...
    public async Task<IReadOnlyCollection<JwtClaimDto>> GetJwtClaimsAsync(Guid userId, CancellationToken ct = default)
    {
        var user = await _userManager.FindByIdAsync(userId.ToString());
        if (user == null)
        {
            return Array.Empty<JwtClaimDto>();
        }
